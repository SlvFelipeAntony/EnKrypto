...
if (isset($_GET['updpasswords'])) { //botão
    if (count($_POST) > 0) {
        $dados = $_POST;
        $updSQL = "UPDATE passwords SET name = ?, url = ?, username = ?, email = ?, pass = ?, idFolder = ? WHERE idPass = ? and idUser = '" . $_SESSION['idUser'] . "'";
...
<div class="container-fluid rounded bg-dark p-3 mb-2">
    <form class="text-bg-dark" action="#" method="post">
        <?php foreach ($records as $record) : ?>
            <div class="mb-3">
                <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="Nome" maxlength="20" name="name" value="<?= $record['name'] ?>" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3 row">
                <div class="input-group col-md-12">
                    <input type="text" class="form-control password  w-50" placeholder="Senha" aria-label="Recipient's username with two button addons" maxlength="30" name="pass" value="<?= $record['pass'] ?>" required>
